...
2011 population data. Covers data cleaning, an entropy/judgment-weighted composite
index, K-Means market segmentation, a forward-looking momentum layer, and a sales-force
allocation model.
...
## Case context: Trilytics 2026 (Sun Pharma case competition)

**Competition:** Trilytics 2026, an analytics case competition organized in
collaboration with the PGDBA Conclave, IIM Calcutta, with participation from IIM
Calcutta, IIT Kharagpur, and ISI Kolkata, sponsored by Sun Pharma.

**Problem statement:** build a statistical model and framework to generate a
District-Level Market Attractiveness Index (MAI) for the Indian pharmaceutical
industry, covering 700+ districts. The goal was to move beyond traditional market
assessment approaches (competition benchmarking, doctor-universe coverage, internal
sales indicators) toward a comprehensive, data-driven, forward-looking framework that
captures the underlying drivers of pharmaceutical market opportunity at a granular
geographic level.

**Required output:** three district-level index scores — Overall MAI, Chronic Therapy
MAI, and Acute Therapy MAI.

**What we optimized for:**
- **Comprehensiveness** — integrating macroeconomic factors, disease burden, patient
  potential, infrastructure, provider availability, affordability, healthcare spending,
  and forward-looking demand indicators, not just a current-state snapshot.
- **Statistical rigor and explainability** — the evaluation criteria explicitly weighted
  "MAI Framework, Model Design & Validation" and "Data, Variables & Assumptions" alongside
  business actionability, so the model needed to be methodologically defensible, not just
  directionally reasonable.
- **Business actionability** — the framework needed to directly support real commercial
  decisions (sales-force deployment, market prioritization, territory planning), not
  remain a purely academic exercise.
- **Replicability and scalability** — the same methodology needed to generalize cleanly
  across all 700+ districts and be reusable for future refreshes, not a one-off,
  hand-tuned output.

**Our extension beyond the brief:** a fourth index, the **Chronic+Acute Overlap Index**,
to capture diseases (e.g. TB, COPD) that carry both a chronic management burden and
recurring acute exacerbations — a dimension the standard two-way chronic/acute split
doesn't represent on its own.

**Evaluation criteria (equal weight):** Data/Variables/Assumptions quality; MAI
Framework/Model Design/Validation rigor; Insights/Business Actionability; and PPT
Communication/Visualization/Innovation.
